...
Comprehensive SQL queries for all safety KPIs from EI Tech App data
"""
...
class EITechKPIQueries:
    """SQL queries for EI Tech App KPIs"""
    ...
    def get_events_per_time_period(self, period: str = 'month') -> List[Dict]:
        """Events per time period using date_of_unsafe_event and reported_date"""
        if period == 'month':
    ...
    def get_events_by_region_country_division(self) -> List[Dict]:
        """Events by region, country_name, division, department"""
        query = f"""
        SELECT
            region,
            country_name,
        ...
        OR division IS NOT NULL OR department IS NOT NULL
    ...
    def get_events_by_unsafe_event_location(self) -> List[Dict]:
        """Events by unsafe_event_location"""
        query = f"""
        SELECT
    ...
    def get_action_completion_rate(self) -> Dict[str, Any]:
        """Action completion rate (count of filled action_description_1 to 5)"""
        query = f"""
        SELECT
    ...
    def get_all_kpis(self) -> Dict[str, Any]:
        """Execute essential KPI queries optimized for analysis (18 Core KPIs)"""
        try:
            logger.info("Executing essential EI Tech KPI queries...")
            ...
            results = {
                ...
                # ==================== ENHANCED INSIGHTS FOR OPERATIONAL INTELLIGENCE ====================
                "operational_alerts_regional": self.get_regional_operational_alerts(),
                "branch_workload_alerts": self.get_branch_workload_alerts(),
                "violation_clusters_with_reasons": self.get_violation_clusters_with_reasons(),
                "location_incident_clusters": self.get_location_incident_clusters(),
                "staff_performance_analysis": self.get_staff_performance_analysis(),
                "high_performing_staff": self.get_high_performing_staff(),
                "resource_optimization_patterns": self.get_resource_optimization_patterns(),
                "seasonal_resource_recommendations": self.get_seasonal_resource_recommendations(),
            }
    ...
    def get_unsafe_acts_and_conditions_analysis(self) -> Dict[str, Any]:
        """Analysis of unsafe acts and conditions"""
        query = f"""
        SELECT
    ...
    def get_reporting_delay_analysis(self) -> Dict[str, Any]:
        """Analyze reporting delays and patterns"""
        query = f"""
        SELECT
    ...
    def get_action_effectiveness_analysis(self) -> Dict[str, Any]:
        """Analyze action plan effectiveness and completeness"""
        query = f"""
        SELECT
            COUNT(*) as total_events,
    ...
    def get_high_risk_location_analysis(self) -> List[Dict]:
        """Identify high-risk locations and unsafe event hotspots"""
        query = f"""
        SELECT
            unsafe_event_location,
            COUNT(*) as total_incidents,
    ...
    def get_time_of_day_incident_patterns(self) -> List[Dict]:
        """Fixed time of day incident patterns with improved time parsing"""
        query = f"""
        SELECT
            CASE
        ...
        FROM {self.table_name}
        WHERE time_of_unsafe_event IS NOT NULL AND time_of_unsafe_event != ''
        GROUP BY
            CASE
                WHEN time_of_unsafe_event IS NULL OR time_of_unsafe_event = '' THEN 'Unknown'
                WHEN time_of_unsafe_event ~ '^([0-9]|1[0-9]|2[0-3]):[0-5][0-9]' THEN
    ...
    def get_regional_safety_performance(self) -> List[Dict]:
        """Comprehensive regional safety performance analysis"""
        query = f"""
        SELECT
            region,
            country_name,
